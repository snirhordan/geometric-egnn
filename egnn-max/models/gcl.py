...
import torch
from qm9.sort import embed_vec_sort, calc_ug, prep_vec_for_embed, count_num_vecs_cloud
...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_att_dim=0, act_fn=nn.ReLU(), recurrent=True, coords_weight=1.0, attention=False, clamp=False, norm_diff=False, tanh=False,sep_nf=1):
        ...
        edge_coords_nf = 1 + sep_nf
        self.global_num = 30
        self.embed_sum = embed_vec_sort( self.hidden_nf, self.global_num, self.hidden_nf, varying_set_size=True, learnable_weights=True )
    ...
    def edge_model(self, source, target, radial, embedding,edge_attr):
        if edge_attr is None:  # Unused.
            out = torch.cat([source, target, radial, embedding], dim=1)
        else:
            out = torch.cat([source, target, radial, embedding,edge_attr], dim=1)
    ...
    def node_model(self, x, edge_index, edge_attr, node_attr):
        row, col = edge_index
        agg = self.unsorted_segment_embed(edge_attr, row, num_segments=x.size(0))
    ...
    def unsorted_segment_embed(self, data, segment_ids, num_segments):
        ...
        pad = torch.zeros(data.size(0), data.size(1), self.global_num - data.size(2), device=device)
        data_pad = torch.cat([data, pad], dim=2)
        non_empty_mask = data_pad.abs().sum(dim=1).bool().int().reshape(data_pad.size(0),1, self.global_num)
        data = torch.cat([non_empty_mask, data_pad], dim=1)
        embedding = self.embed_sum(data)
        return embedding
