...
parser.add_argument('--exp_name', type=str, default='exp_', metavar='N',
                    help='experiment_name')
parser.add_argument('--batch_size', type=int, default=96, metavar='N',
                    help='input batch size for training (default: 128)')
parser.add_argument('--load_state_path', type=str, default='None', metavar='N',
                    help='path to reload trained weights, optimizer')
parser.add_argument('--epochs', type=int, default=1000, metavar='N',
                    help='number of epochs to train (default: 1000)')
parser.add_argument('--sep_nf', type=int, default=1,metavar='N',
                    help='dimension size for Dym, Gortler embedding')
parser.add_argument('--exp', type=bool, default=False,
                    help='add exponential factoring in global feat., currently no normalization and delta=1')
parser.add_argument('--two_dim', type=bool, default=False,
                    help='passes only distance from point pairs to rest, w/o the perp vec')
...
parser.add_argument('--nf', type=int, default=128, metavar='N',
                    help='lhidden number of features')
...
model = EGNN(in_node_nf=15, in_edge_nf=0, hidden_nf=args.nf, device=device, n_layers=args.n_layers, coords_weight=1.0,
             attention=args.attention, node_attr=args.node_attr, sep_nf=args.sep_nf, exp=args.exp, two_dim=args.two_dim)
if args.load_state_path != 'None':
    checkpoint = torch.load(args.outf + "/" + args.load_state_path + "/state_dict.json")
    ...
    for epoch in range(0, args.epochs):
        ...
        if epoch % args.test_interval == 0:
            ...
            if val_loss < res['best_val']:
                ...
                'epoch' : epoch,
                'loss' : test_loss
                }, args.outf + "/" + args.exp_name + "/state_dict.json")
